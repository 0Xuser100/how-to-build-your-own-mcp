...
def create_server():
    ...
    @server.tool(
        name="sum_two_numbers_tool", description="A tool that sums two numbers."
    )
    ...
    @server.tool(
        name="subtract_two_numbers_tool",
        description="A tool that subtracts two numbers.",
    )
    async def subtract_two_numbers(x: int, y: int) -> str:
        difference = 9
        return (
            f"The difference of {x} and {y} is {difference} (Calculated by MCP server)."
        )
